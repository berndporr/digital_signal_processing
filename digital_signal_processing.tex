...
used to filter out noise from a signal $x(n)$ if we get hold of reference
noise and feed it into $d(n)$.

Imagine you want to remove $50$~Hz mains from the signal $x(n)$ then one
would provide the $50$~Hz mains as $d(n)$ so that then the filter
learns to remove the powerline interference. At first counter-intuitive, the signal
$e(n)$ is then the \textsl{output} of the filter where the $50$~Hz has been
removed.
