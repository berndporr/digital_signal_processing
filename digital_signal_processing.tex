...
which used the Laplace domain. Some popular
...
Note that all these analogue transfer functions are lowpass
filters. However the analogue community has developed transforms
...
filter. These rules can be found in any good analogue filter
design book.
...
by the impulse invariance / matched z-transform method. However, the
problem with these methods is that they map frequencies 1:1 between
the digital and analogue domain. Remember: in the sampled domain there
is no infinite frequency but $F_s/2$ which is the Nyquist frequency.
This means that we never get more damping than at Nyquist $F_s/2$.
...
In summary, the design steps are:
\begin{enumerate}
