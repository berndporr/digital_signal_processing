...
{\Large Bernd Porr\footnote{Thanks to Fiona Baxter for typing up the handwritten
lecture notes.}}

\smallskip

\texttt{bernd.porr@gla.ac.uk}
...
In contrast to the lecture material it focusses more on the theoretical
aspects of it and its analytical derivations. The (video-) lectures
cover a lot more practical programming examples when switching to Python and C.
...
Also general purpose boards such as the Raspberry PI are great
platforms for DSP. For example, add a USB sound card such as the i-mic
to the Rapberry PI and you can do realtime sound filtering.
...
a plot how analogue frequencies map onto sampled frequencies.
...
& = & \int_{0}^{\infty} h(t) e^{e-st} e^{e-st} dt \\
...
\begin{eqnarray}
x(t) & = & \delta(t) \qquad \leftarrow \mbox{deta pulse} \\
...
X(s) & = & \sum_{n = 0}^{\infty} x(n) e^{-snT} \\
     ...
     & = & \sum_{n = 0}^{\infty} x(n) \underbrace{(e^{-sT})^{n}}_{z^{-1} = e^{-s}} \\
...
Let's substitute $s$ by $z$: $z^{-1} = e^{-s}$ which
...
set up a digital filter (see Fig.~\ref{FIRfilter}).
...
IIR filter stands for Infinite Impulse Response. Such filters
are implemented as recursive filters. We will see that
impulse responses from exponentials can easily be implemented
as a recursive filter.

\subsubsection{Introduction}
We'll try to find a recursive version of a discrete filter. To
demonstrate how this works we take a very simple analogue filter and
sample it. This example can then be generalised to more complex
situations.
...
\subsubsection{General form of filters}
...
\subsubsection{Filter design from analogue filters}
Traditionally digital filters were designed from analogue
filters by transforming from the continuous domain to the
sampled domain. This has historical reasons because before digital
signal processing became popular most designs were analogue
and used Transfer functions in the Laplace domain. Some popular
analogue transfer functions are:

\begin{itemize}
\item {\bf Butterworth:} All poles lie on the left half plane
equally distributed on a half circle with radius $\Omega$.
\begin{itemize} 
\item monotonic  
\item only poles, no zeros  
\item standard filter for many applications  
\item no constant group delay!  
\end{itemize}
...
Note that all these transfer functions generate complex conjugate pole
pairs (plus one real pole if of odd order) which suggests a chain of
2nd order structures straight away.

These analogue transfer functions need to be transformed in the
sampled domain. This could be done by the impulse invariance / matched
z-transform method but the problem with these methods is 
that they map frequencies
1:1 between the digital and analogue domain. Remember: in the sampled
domain there is no infinite frequency but only $F_s/2$. It ends at the Nyquist
frequency which means that we never get more damping than at $F_s/2$.
This is especially a problem for lowpass filters where damping
increases the higher the frequency.

The solution is to map {\bf all} analogue frequencies from $0 \ldots \infty$
to $0\ldots 0.5$ (normalised frequency) in a non-linear way:
\begin{equation} 
...
Let's test if the mapping works. In the analogue domain the
...
Note, that the bilinear transform is a 
{\bf non}linear mapping of the frequency:
\begin{equation}
\Omega = \frac{2}{T} tan \frac{\omega}{2}
\end{equation}
So, the cut-off frequency of our analogue filter is changed
by the bilinear transformation. Consequently, we need a pre-warp of the
analogue filter frequency:
\begin{equation}
...
\item Pre-warp $\omega_{c} \rightarrow \Omega_{c}$ with Eq.~\ref{prewarp}
...
$H(s)$ by $\frac{2}{T} \frac{z - 1}{z + 1}$ 
to obtain the digital filter $H(z)$
\item Change the transfer function $H(z)$ so that it contains only
negative powers of $z$ ($z^{-1}, z^{-2}, \ldots$) which can be
interpreted as delay lines.
\item Build your IIR filter!
\end{enumerate}

Note that the bilinear is a real \textsl{pain}. Nobody wants to
transform high order analogue transfer functions $H(s)$ to the
$z$ domain. The solution lies again in the 2nd order terms:
Split up the analogue transfer function $H(s)$  in a chain of 2nd order filters
$H(s) = H_1(s) H_2(s) H_3(s) \ldots$. Using this strategy you only
need to calculate the bilinear transform once for a 2nd order system
(or if the order is odd then also for a 1st order one) but then
there is no need to do any more painful bilinear transforms. This
