...
\caption{Pole placements of the Butterworth filter. Its analogue
  cutoff frequency is $\Omega$. This filter can be implemented
...
\item {\bf Butterworth:} All poles lie on the left half plane
  equally distributed on a half circle with radius $\Omega$ which
  is shown in Fig.~\ref{butterworth_poles}.
\begin{itemize} 
\item monotonic  
...
calculate for anything above order two.  Nobody wants to transform
high order analogue transfer functions $H(s)$ to the $H(z)$
...
one real pole if of odd order) which suggest a chain of 2nd order IIR
