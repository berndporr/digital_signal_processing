...
In realtime systems and any systems processing data as it
arrives the future samples are not known. This is as in
an analogue system where we send for example a signal into
an amplifier and it needs to process it as it arrives.

However, the Fourier transform is \textsl{not real time}.  
We need the whole signal from the first to the last sample.
For that reason we need to develop a new mathematical framework
which we call ``Causal Signal Processing''. These systems
should ideally react as an analogue system, namely as fast
as possible with little latency as possible.
...
its own nature a realtime causal processor.
